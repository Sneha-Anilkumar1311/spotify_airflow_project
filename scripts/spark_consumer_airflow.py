from pyspark.sql import SparkSession
from pyspark.sql.functions import from_json, col
from pyspark.sql.types import StructType, StringType

schema = StructType() \
    .add("track_name", StringType()) \
    .add("artist", StringType()) \
    .add("album", StringType()) \
    .add("played_at", StringType())

spark = SparkSession.builder \
    .appName("spotify_streaming_airflow") \
    .config(
        "spark.jars.packages",
        "org.apache.spark:spark-sql-kafka-0-10_2.12:3.4.4,org.postgresql:postgresql:42.7.3"
    ) \
    .getOrCreate()

df = spark.read \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
    .option("subscribe", "spotify_topic") \
    .option("startingOffsets", "earliest") \
    .load()

spotify_df = df.select(
    from_json(col("value").cast("string"), schema).alias("data")
).select("data.*")

spotify_df = spotify_df.dropDuplicates(["track_name", "artist", "played_at"])

df_clean = df.dropDuplicates(["played_at"])

df_clean.write \
    .format("jdbc") \
    .option("url", "jdbc:postgresql://localhost:5432/spotifydb") \
    .option("dbtable", "spotify_tracks") \
    .option("user", "postgres") \
    .option("password", "newpassword") \
    .option("driver", "org.postgresql.Driver") \
    .mode("append") \
    .save()

spark.stop()
